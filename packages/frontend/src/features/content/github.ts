...
export async function list_content_paths(): Promise<string[]> {
  ...
  if (!res.ok) {
    throw new Error(`Failed to list content tree: ${res.status}`);
  }
...
export async function fetch_content_text(path: string): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch content file ${path}: ${res.status}`);
  }
  return res.text();
}

export async function fetch_content_binary(
  path: string,
): Promise<ArrayBuffer | null> {
  if (process.env.ALLOW_EMPTY_CONTENT === "1") return null;
  const res = await fetch_content_file(path);
