...
const load_post = async (slug: string): Promise<Post> => {
  const source = await fetch_content_text(`blog/${slug}/post.md`);
  const { frontmatter, content } = parse_blog_post(source);
  return {
    slug,
    meta: {
      title: frontmatter.title,
      description: frontmatter.description,
      // Absolute so it also works in RSS items and OG metadata.
      image: `${env.FRONTEND_URL}/content-assets/blog/${slug}/${frontmatter.image}`,
