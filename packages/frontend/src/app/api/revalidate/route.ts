...
async function verify_signature(body: string, header: string | null): Promise<boolean> {
...
export async function POST(req: Request) {
  ...
  const valid = await verify_signature(body, req.headers.get("x-hub-signature-256"));
  if (!valid) {
